...
@section('content')
    ...
    <div class="profile-container">
        ...
        <form id="userForm" method="post" action="{{ route('updateprofile2', ['id' => $user->id]) }}">
            @csrf
            @method('put')
            ...
            <select name="body_type" style="color: black;" required>
                <option value="Cuting" {{ $user->body_type == 'Cuting' ? 'selected' : '' }} style="color: black;">Cuting
                </option>
                <option value="Normal" {{ $user->body_type == 'Normal' ? 'selected' : '' }} style="color: black;">Normal
                </option>
                <option value="Bulking" {{ $user->body_type == 'Bulking' ? 'selected' : '' }} style="color: black;">Bulking
                </option>
            </select>
